...
​				GET请求：body的值为null  ->  send(null)  **# 通过查询字符串向服务器传参**
...
| 状态吗 |                             说明                             |
| :----: | :----------------------------------------------------------: |
|  200   | 表示服务器正确处理所有的请求以及给出响应**(readyState状态值为4，且服务器状态为200时，才能响应和渲染数据)** |
|  404   |                        请求资源不存在                        |
|  500   |                        服务器内部错误                        |
...
​			**一般系统函数，例如time(),我们通过调用系统函数去完成功能。系统编写，我们调用。**

​			**回调函数，我们编写，系统在满足条件时自动调用。**
...
**将对象转换为字符串，称为序列化；将字符串转换为对象，称为反序列化。**
...
1.编写ajax请求的js代码。代码需要调试时：alert;或console.log。

2.编写与请求中的url所对应的视图函数。

​	2.1 获取前端传递过来的数据。调试时，print打印。结果出现在runserver的终端。

​	2.2 响应回Response对象。

3.客户端没有正确响应怎么调试。

​	通过浏览器的调试窗口，找到网络标签，双击对应的响应状态，

​	在右边出现响应信息，读一下响应信息。根据响应信息调试错误