...
- 模型管理器的使用方法:
 ...
 - 模型管理器类ModelAdmin中实现的高级管理功能
    1. list_display 去控制哪些字段会显示在Admin 的修改列表页面中。
       ...
       ![image-20201118192647434](/home/tarena/.config/Typora/typora-user-images/image-20201118192647434.png)
    
    2. list_display_links 可以控制list_display中的字段是否应该链接到对象的“更改”页面。**(如在此中设置了某项字段的超链接，则无法在list_editable中设置此字段，会报错)**
    
    3. list_filter 设置激活Admin 修改列表页面右侧栏中的过滤器 
    
       ![image-20201118192736908](/home/tarena/.config/Typora/typora-user-images/image-20201118192736908.png)
    
    4. search_fields 设置启用Admin 更改列表页面上的搜索框。  
    
    5. list_editable 设置为模型上的字段名称列表，这将允许在更改列表页面上进行编辑。
    
       c![image-20201118192843934](/home/tarena/.config/Typora/typora-user-images/image-20201118192843934.png)
...
- 用法格式如下:
    ...
    class Book(models.Model):
        ...
        class Meta:
            ...
            2. verbose_name = '单数名' # 例如verbose_name = '图书',后台管理界面/admin中显示为'图书s'
                - 给模型对象的一个易于理解的名称(单数),用于显示在/admin管理界面中
            3. verbose_name_plural = '复数名' # 如要去除'图书s'中的's'复数，需要此函数.
                - 该对象复数形式的名称(复数),用于显示在/admin管理界面中
    ```

    ![image-20201118193333364](/home/tarena/.config/Typora/typora-user-images/image-20201118193333364.png)
...
- 练习:
    - 将Author 模型类加入后台管理
      ...
      # 终端输入：python3 manage.py shell,创建内容：
      In [1]: from oto.models import *
      In [2]: author1 = Author.objects.create(name='王老师')
      In [4]: author2 = Author.objects.create(name='魏老师')
      In [6]: author3 = Author.objects.create(name='小泽老师')
      ```
    ...
    - 用后台管理程序 添加三条 Author 记录
      ...
      ![image-20201118195354533](/home/tarena/.config/Typora/typora-user-images/image-20201118195354533.png)
    
    - 修改其中一条记录的年龄 - Author
    
    - 删除最后一条添加的记录 - Author
    
      ![image-20201118195446925](/home/tarena/.config/Typora/typora-user-images/image-20201118195446925.png)
...
在一对一的关系中，关联属性放到哪个类中都可以。但是一旦放到某一类中，例如放到B类中，这时，B类对应的表示从表，另外一个A表是主表。
...
3. 创建一对一的数据记录
    ```python
    # 在终端python3 manage.py shell中创建
    from .models import *
    author1 = Author.objects.create(name='王老师')
    # 写法一：
    wife1 = Wife.objects.create(name='王夫人', author=author1)  # 关联王老师
    # 写法二：
    wife1 = Wife.objects.create(name='王夫人',author_id=author1.id)
...
4. 创建数据
    ...
    ```python
    #在终端python3 manage.py shell中创建,先创建 '一' ，再创建 '多'
    from .models import *
...
5. 数据查询
   ...
   **总结：如果在管理后台中使用模型类，最好重写`__str__`方法。**
   
   **在后台管理中，增加对出版社和图书的管理【10分钟】**
...
- 火狐浏览器 可能通过开发者工具的 存储 -> Cookie
    ...
    - cookies 在浏览器上是以键-值对的形式进行存储的，键和值都是以ASC	II字符串的形存储**(不能是中文字符串)**
    - cookies中的数据是按域存储隔离的，不同的域之间无法访问**(按域隔离)**

    - cookies 的内部的数据会在每次访问此网址时都会携带到服务器端，如果cookies过大会降低响应速度**(自动提交)**
