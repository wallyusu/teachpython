...
​	2.注意缓存数据与被缓存的数据一致性。为了避免数据的不一致，在修改了数据后，要及时清除缓存。

​	3.有些场景要求数据完全的实时的一致性；有些场景允许短时间的不一致。根据具体场景来看。
...
在视图View中使用cache **(加一个装饰器@cache_page()即可，对整个结果进行缓存。但缓存不灵活，1.无法精确其中一部分数据。2.想把缓存删掉，无法直接删除。如需单独对某个细项缓存，可使用缓存api)**
...
## 中间件 Middleware
- 中间件是 Django 请求/响应处理的钩子框架(回调函数)。它是一个轻量级的、低级的“插件”系统，用于全局改变 Django 的输入或输出。
...
- 中间件类:
    ...
    - 中间件类须实现下列五个方法中的一个或多个:
        ...
        - `def process_exception(self, request, exception):` 当处理过程中抛出异常时调用，返回一个HttpResponse对象 **在该函数中，将异常信息记录到日志中，方便管理员查看。**
...
def make_csv_view(request):
    # 1. 指定响应内容的类型
    response = HttpResponse(content_type='text/csv')  # 创建响应对象时 指定响应内容的类型
    # 2. 指定以附件的形式，另存为...
