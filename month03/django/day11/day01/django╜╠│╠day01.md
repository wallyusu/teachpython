...
- 一个重量级的 Python Web框架，Django 配备了常用的大部分组件
    1. 基本配置(setting.py)
    1. 路由系统(url统一资源定位器),用户就是在地址栏输入url去访问服务器的某个资源的。
    1. 原生HTML模板系统(与我们前面所讲的html页面类似，不同的是，可以包含动态数据)
    1. 视图 view(处理用户的url请求，并响应)
    1. Model模型,数据库连接和ORM数据库管理(提供动态数据的)
    1. 中间件(Django提供的不同功能模块)
    1. Cookie & Seesion(维持http状态，例如：用于登录认证等等)
    1. 分页
    1. 数据库后台管理系统admin(前台和后台)
...
- URL的一般语法格式为：
    ```
    protocol :// hostname[:port] / path [?query][#fragment] # 锚点
    ```
- 如:
    ```
...
- 练习:
    ...
    - 如:
        ...
        结论：1.两个url可以对应同一个视图函数，path转换器的参数传递采用的是关键字传参，与位置无关。
        
        除了可以使用**path转换器**传递参数外，还可以会用**查询字符串**传参。
...
- 下面是常见的HTTP状态码：
    ...
    - 301 - 永久重定向-资源（网页等）被永久转移到其它URL
      ...
      响应头Location包含新的url，浏览器收到响应后，再次向新的url发送请求。这样的话，就可以访问新的页面了。
...
- GET请求方式中，如果有数据需要传递给服务器，通常会用查询字符串(Query String)传递    【注意：不要传递敏感数据】
    ...
    - 服务器端接收参数
        ...
        # 如果参数名不存在，没有默认值，返回None,有默认值返回默认值
        request.GET.get('参数名','默认值')
        # 参数名允许重复，获取多个值，用&连接，保存在列表中
